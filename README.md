...
### 1.1 Structural Estimation (mle_analyzis_2005_2023.py)
...
- **Inputs**: tech_intensive_panel2005-2023.csv and industrial_sector_panel_2005_2023.csv.
...
2. **Step 1**: Run `mle_analyzis_2005_2023.py` to generate expectations.
3. **Step 2**: Run 'mle_comparative_graphs.py' to generate comparative ((employment, pp&e:fig.1, 2)and ebit margin) graphs
3. **Step 3**: Run `mle_analyzis_global_histogram_fit.py` to validate the distribution fits.
4. **Step 4**: Run the sector-specific correlation scripts to populate the article's tables.
5. **Step 5**: Run the comparison script to finalize the hypothesis testing.
